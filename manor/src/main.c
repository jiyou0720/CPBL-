    ...
    while(1){
        ...
        if(ch=='\n'||ch=='\r'||ch==KEY_ENTER){
            ...
            if(sel==3){
                ...
                mvprintw(11,4,"[P]          회복 물약 사용 (HP +3)");
                mvprintw(10,4,"[TAB]        업적 확인");
                mvprintw(12,4,"[ESC]        일시정지 메뉴");
...
static void handle_input(GameState *gs, int ch){
    ...
    case MODE_MAP:
        ...
        case 'p':case 'P': if(state_has_item(gs, ITEM_POTION))
            {
                if(gs->hp < HP_MAX)
                {
                    gs->hp += 3;
                    ...
                    state_remove_item(gs, ITEM_POTION);

                    state_set_msg(gs, "회복 물약을 사용했다. HP +3");
                }
                else
                {
                    state_set_msg(gs, "체력이 이미 가득 차 있습니다.");
                }
            }
            else
            {
                state_set_msg(gs, "회복 물약이 없습니다.");
            }
